...
const VirtualGridProvider = forwardRef((props, ref) => {
    ...
    const visibleRowCount = Math.min(props.rowCount - startRow, Math.ceil(h / props.cellHeight));
    const offsetY = startRow * props.cellHeight; //Vertical transform
    const totalHeight = props.rowCount * props.cellHeight;
    ...
    const getRowIndex = (AY) => {
        ...
        rowIndex = Math.min(Math.max(0, rowIndex), props.rowCount - 1);
    ...
    const updateSelection = (arr) => {
        let mi0 = Math.min(Math.max(arr[0], 0), props.rowCount - 1); //Row index
    ...
    const handleHeaderClick = (e) => {
        ...
        setPos([props.rowCount - 1, colIndex]);
    }
    ...
    const handleClickFixFirstColumn = (e) => {
        e.stopPropagation();
        setAnchor([0, 0]);
        setPos([props.rowCount - 1, props.headers.length - 1]);
    }
        ...
        if (e.shiftKey) {
            ...
            if ((selectedCells.length !== 0) || (cellEditablePos !== null)) {
                ...
                if (!['ArrowRight', 'ArrowLeft', 'ArrowUp', 'ArrowDown'].includes(e.code)) {
                    ...
                    roStatic = Math.min(Math.max(0, roStatic), props.rowCount - 1);
    ...
    const normalVisibleRows = useMemo(() => {
        ...
        for (i = 0; i < visibleRowCount; i++) {
            ...
            if (!(props.fixRows.includes(i + startRow))) {
                normalRows.push(
                    <div
                        onMouseDown={(e) => handleMouseDown2(e)}
                        onClick={(e) => handleClick2(e)}
                        onDragStart={(e) => dragStart(e, i + startRow)}
                        onDragEnter={(e) => dragEnter(e, i + startRow)}
                        onDragEnd={(e) => drop(e)}
        ...
        return normalRows;
    }, [props.rowCount, startRow, visibleRowCount, cellResize, selectedCells, w, h, showSummary, showFilter, scrollTop]);
            ...
            <div className={"filterRowBlock"}>
                <div className="normalCellContainer">
                    ...
                    {props.headers.map((hc, col) => (
                        <div className={"headerCell"} style={{width: props.onColWidth(col)}}>
                            <props.FilterRow
                                key={col}
                                index={col}
                            />
    ...
    return (<div
        tabIndex={0} className={"container"} style={{width: w, height: h}} ref={refScroll}
        //events
    ...
    >
                    ...
                    <div className="stickyCellContainer">
                        {/*Headers */}
                        {
                            (props.headers.length === 1) ?
                                // Tree
                                <div className={"headerCellSingle"} style={{paddingLeft: 8}}>{props.headers[0]}</div>
                                :
                                <>
                                    {/*(0,0) fix column in top left corner */}
        ...
        {/*Normal rows: content */}
        <div className="rowsContainer" style={{willChange: "transform", height: totalHeight}}>
